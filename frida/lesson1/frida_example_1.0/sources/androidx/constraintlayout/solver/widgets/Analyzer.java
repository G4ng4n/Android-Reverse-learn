...
public class Analyzer {
    ...
    private static int getMaxDimensionTraversal(ConstraintWidget constraintWidget, int i, boolean z, int i2) {
        ...
        while (it.hasNext()) {
            i13 = Math.max(i13, getMaxDimensionTraversal(((ResolutionAnchor) ((ResolutionNode) it.next())).myAnchor.mOwner, i12, z2, i15));
        }
        int i16 = 0;
        for (Iterator it2 = constraintWidget2.mListAnchors[i7].getResolutionNode().dependents.iterator(); it2.hasNext(); it2 = it2) {
            i16 = Math.max(i16, getMaxDimensionTraversal(((ResolutionAnchor) ((ResolutionNode) it2.next())).myAnchor.mOwner, i12, z2, width2 + i15));
        }
        if (z3) {
            i13 -= i6;
            width = i16 + i5;
        ...
        if (z2) {
            Optimizer.setOptimizedWidget(constraintWidget2, i12, i15);
            constraintWidget2.setFrame(i15, i20, i12);
        } else {
            constraintWidget2.mBelongingGroup.addWidgetsToSet(constraintWidget2, i12);
            constraintWidget2.setRelativePositioning(i15, i12);
        }
        ...
        if (constraintWidget2.mListAnchors[i4].mTarget != null && constraintWidget2.mListAnchors[i9].mTarget != null && constraintWidget2.mListAnchors[i4].mTarget.mOwner == (parent = constraintWidget.getParent()) && constraintWidget2.mListAnchors[i9].mTarget.mOwner == parent) {
            constraintWidget2.mBelongingGroup.addWidgetsToSet(constraintWidget2, i12);
        }
        return max;
    }
